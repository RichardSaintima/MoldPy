...
def generar_plantilla_fastapi_firebase(target_path: str, nombre: str, autor: str):
    """Orquesta la creación de la plantilla FastAPI + NoSQL Serverless."""
    target_p = Path(target_path)
    app_dir = target_p / "app"
    routers_dir = app_dir / "routers"
    routers_dir.mkdir(parents=True, exist_ok=True)

    # 1. Copiar archivos estáticos si existen (Dockerfile, etc.)
    ...
    # 2. Generar módulos
    crear_config_files(str(app_dir), str(target_p), nombre)
    crear_db_files(str(app_dir), str(routers_dir))
...
def _crear_main(app_dir: str, nombre: str, autor: str):
    main_content = dedent(f'''\
        ...
        @app.get("/")
        def root():
            return {{"message": "API REST NoSQL Serverless activa"}}
    ''')
...
def _crear_requirements(target_path: str, autor: str):
    req_content = dedent(f'''\
        # Dependencias para API REST NoSQL Serverless
